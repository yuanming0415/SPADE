        ...
        if echo "$line" | grep -q "$keyword"; then
            ...
            if [ "$output" = "位置需要更换" ]; then
                ...
                if [ -f "$filename" ]; then
                    ...
                    # 获取倒数第二行的最后两个单词
                    last_two_words=$(echo "$penultimate_line" | awk '{print $(NF-1), $NF}')
                    
                    # 删除倒数第二行的最后两个单词
                    new_penultimate_line=$(echo "$penultimate_line" | awk 'NF{NF-=2}1')
                    
                    # 获取最后一行
                    last_line=$(tail -n 1 "$filename")
                    
                    # 将倒数第二行的最后两个单词追加到最后一行的末尾
                    new_last_line="${last_line} ${last_two_words}"

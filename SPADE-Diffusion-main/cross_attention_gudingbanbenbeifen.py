        ...
        def matsepcalc(x,contexts,pn,divide):
            ...
            for drow in self.split_ratio:
                ...
                for dcell in drow.cols:
                    ...
                    '''
                    if x.shape[1] == 4096:
                        #print("整体溃散111111111111111111111111111")
                        if hstart == 0.5:
                            hstart = hstart + 0.1
                        elif hend == 0.5:
                            hend = hend - 0.1
                        ...
                        #print("outout",out.shape)
                    elif x.shape[1] == 1024:
                        #print("整体溃散111111111111111111111111111111111111111111111111111111111111111111111111111111")
                        if hstart == 0.5:
                            hstart = hstart + 0.1
                        elif hend == 0.5:
                            hend = hend - 0.1
                        ...
                        #print("分开扩散1111111111111111111111111111")
                    ...
                    if x.shape[1] == 4096 or x.shape[1] == 1024:
                        #print("整体溃散22222222222222222222222222222222222222")
                        if drow.start == 0 and drow.end == 1:
                            ...
                            if drow.start == 0:
                                ...
                                out = out[:,int(latent_h*modstart) + addout:int(latent_h*drow.end),
                                    int(latent_w*dcell.start) + addin:int(latent_w*dcell.end),:]
                        #print("要求outout",out.shape)
                    else:
                        #print("分开扩散22222222222222222222222222222222222222")
                        #print("drow.start",drow.start)
                        #print("drow.end",drow.end)
                        #print("dcell.start",dcell.start)
                        #print("dcell.end",dcell.end)
                        out = out[:,int(latent_h*drow.start) + addout:int(latent_h*drow.end),
                                int(latent_w*dcell.start) + addin:int(latent_w*dcell.end),:]
                    if self.usebase :
                        #print("dcell.base",dcell.base)
                        ...
                        #print("outb_toutb_t",outb_t.shape)
                        if x.shape[1] == 4096 or x.shape[1] == 1024:
                            #print("整体溃散33333333333333333333333333333333333333")
                            if drow.start == 0 and drow.end == 1:
                                #print("左右结构")
                                if dcell.start == 0:
                                    ...
                                    if (out_full.size()[2] == 32):
                                        ...
                                        #print("检查out_full",out_full[:, :, :19, :].shape)
                                        #print("替补out_full",out_full[:, :, 19:, :].shape)
                                    elif (out_full.size()[2] == 16):
                                        out_full[:, :, :12, :] = out
                                        out_full[:, :, 12:, :] = outb_t[:, :, 12:, :]
                                        #print("检查out_full",out_full[:, :, :9, :].shape)
                                        #print("替补out_full",out_full[:, :, 9:, :].shape)
                                    out_full = out_full * (1 - dcell.base) + outb_t * dcell.base
                                    #if out_full.size()[2] == 32:
                                    #    out_full[:, :, 25:, :] = 0  # 32 通道时，25 之后的通道置零
                                    #elif out_full.size()[2] == 16:
                                    #    out_full[:, :, 12:, :] = 0  # 16 通道时，12 之后的通道置零

                                    out = out_full
                                    #print("outout1",out.shape)
                                    #print("outout1",out)
                                elif dcell.end == 1:
                                    out_full = torch.zeros_like(outb_t)
                                    if (out_full.size()[2] == 32):
                                        ...
                                        #print("检查out_full",out_full[:, :, -20:, :].shape)
                                        #print("替补out_full",out_full[:, :, :n-20, :].shape)
                                    elif (out_full.size()[2] == 16):
                                        n = out_full.size(2)
                                        out_full[:, :, -13:, :] = out
                                        out_full[:, :, :n-13, :] = outb_t[:, :, :n-13, :]
                                        #print("检查out_full",out_full[:, :, -10:, :].shape)
                                        #print("替补out_full",out_full[:, :, :n-10, :].shape)
                                    out_full = out_full * (1 - dcell.base) + outb_t * dcell.base
                                    #if out_full.size()[2] == 32:
                                    #    out_full[:, :, :-26, :] = 0  # 总长度32 -26=6，前6个位置清零
                                    #elif out_full.size()[2] == 16:
                                    #    out_full[:, :, :-13, :] = 0  # 总长度16 -13=3，前3个位置清零
                                    out = out_full
                                    #print("outout2",out.shape)
                                    #print("outout2",out)
                            elif dcell.start == 0 and dcell.end == 1:
                                #print("上下结构")
                                if drow.start == 0:
                                    ...
                                    out_full = out_full * (1 - dcell.base) + outb_t * dcell.base
                                    #if out_full.size()[1] == 32:
                                    #    out_full[:, :-26, :, :] = 0  # 32 通道时，25 之后的通道置零
                                    #elif out_full.size()[1] == 16:
                                    #    out_full[:, :-13, :, :] = 0  # 16 通道时，12 之后的通道置零
                                    out = out_full
                                    #print("outout5",out.shape)
                                    #print("outout5",out)
                            #print("outout",out.shape)
                        else:
                            #print("分开扩散")
                            #print("outout",out.shape)
                            #print("outbtoutbt",outb_t.shape)
                            out = out * (1 - dcell.base) + outb_t * dcell.base
                        #print("outout3",out.shape)
                        #print("outout3",out)
                    v_states.append(out)
                    #print("out1",out.shape)
                output_x = torch.cat(v_states,dim = 2)
                #print("out2",output_x.shape)
                h_states.append(output_x)
            output_x = torch.cat(h_states,dim = 1) 
            #if x.shape[1] == 4096 or x.shape[1] == 1024:
                #print("out3",output_x.shape)
                #print("out3",output_x)

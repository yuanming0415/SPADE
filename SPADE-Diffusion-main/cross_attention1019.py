            ...
            for drow in self.split_ratio:
                ...
                for dcell in drow.cols:
                    ...
                    if cnet_ext > 0:
                        ...
                        context = torch.cat([context,contexts[:,-cnet_ext:,:]],dim = 1)
                    i = i + 1 + dcell.breaks
                    #print("分区方式",hstart,hend,wstart,wend)
                    #print("others")
                    #print(x.shape)
                    if (x.shape[1] == 16384 and x.shape[2] == 640):
                        if hstart == 0.5:
                            ...
                            wend = wend
                        #print("到这里来了?",hstart,hend,wstart,wend)
                        ...
                        if hstart == 0.5:
                            ...
                            wend = wend
                        #print("到这里来了2?",hstart,hend,wstart,wend)
                    ...
                    if (x.shape[1] == 16384 and x.shape[2] == 640) or (x.shape[1] == 4096 and x.shape[2] == 1280):
                        ...
                        #print("drow.start",dcell.start)
                        #print(latent_h*drow.start)
                        #print("drow.end",dcell.end)
                        #print(latent_h*drow.end)
                        #print("dcell.start",dcell.start)
                        #print(latent_w*dcell.start)
                        #print("dcell.end",dcell.end)
                        #print(latent_w*dcell.end)
                        ...
                        #print("确认区域扩散")
                        #print("rstart,rend,cstart,cend",drow.start,drow.end,dcell.start,dcell.end)
                        out = out[:,int(latent_h*drow.start) + addout:int(latent_h*drow.end),
                                int(latent_w*dcell.start) + addin:int(latent_w*dcell.end),:]
                        #print("裁剪区域扩散后",out.shape)
                    if self.usebase :
                        #print("bbb",drow.start,drow.end,dcell.start,dcell.end)
                        #print("ccc",addout,addin)
                        ...
                        #print("把base的裁剪一部分",outb_t.shape)
                        if x.shape[1] == 4096 or x.shape[1] == 1024:
                        #    if drow.start == 0 and drow.end == 1:
                        #        if dcell.start == 0:
                        ...
                        #            out = out_full
                            ...
                            #print("11111111111111",out.shape)
                        else:
                            #print(out.shape)
                            out = out * (1 - dcell.base) + outb_t * dcell.base
                            #print("222222222222222",out.shape)
                    v_states.append(out)
                output_x = torch.cat(v_states,dim = 2)
                #print("拼列后",output_x.shape)
                h_states.append(output_x)
                #分区for循环就到这里了
            output_x = torch.cat(h_states,dim = 1) 
            #print("拼行后",output_x.shape)
